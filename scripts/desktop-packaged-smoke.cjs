...
const executable = path.join(projectRoot, "out", "desktop", "win-unpacked", "BossAI Funding.exe");
if (!fs.existsSync(executable)) {
  console.error("BOSSAI_FUNDING_PACKAGED_SMOKE_MISSING", executable);
  process.exit(1);
}

const tempUserData = fs.mkdtempSync(path.join(os.tmpdir(), "bossai-funding-packaged-smoke-"));
let stdout = "";
let stderr = "";
...
child.on("error", (error) => {
  ...
  cleanup();
  console.error("BOSSAI_FUNDING_PACKAGED_PROCESS_ERROR", error);
  process.exitCode = 1;
});
...
child.on("close", (code) => {
  ...
  console.log("BOSSAI_FUNDING_PACKAGED_SMOKE", JSON.stringify({
    passed,
    code,
    timedOut,
    databaseInsideTemp,
